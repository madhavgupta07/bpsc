...
export default function NoteDetail() {
  ...
  return (
    <div className="container-app py-8">
      <Seo
        title={`${L(note.title)} Notes — Bihar STET & BPSC TRE CS Chapter ${note.num}`}
        description={`${L(note.intro)} Complete chapter ${note.num} notes in Hindi & English for Bihar STET & BPSC TRE Computer Science with explanations and practice tips.`}
        path={`/notes/${num}`}
        keywords={`${note.title[0]} notes, ${note.title[1]} नोट्स, Bihar STET Chapter ${note.num} notes, BPSC TRE CS ${note.title[0]}, STET CS ${note.title[0]}`}
      />
      {/* Header */}
